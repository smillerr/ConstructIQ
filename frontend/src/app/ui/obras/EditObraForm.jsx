...
export default function EditObraForm({ obraId }) {
  ...
  const handleCreateObrasForm = async (data) => {
    ...
    const editedObra = await editConstruction(obraId, obraData)
    if (editedObra) {
      await uploadConstructionImage(editedObra.id, img_obra)
  ...
  useEffect(() => {
    const fetchFormData = async () => {
      ...
      setValue('estado', obraData.estado)
      setValue('tipo_obra', obraData.tipo_obra)
      setValue('fecha_inicio', obraData.fecha_inicio)
      setValue('fecha_final', obraData.fecha_final)
      setValue('id_director', obraData.id_director.id)
      setValue(
        'id_capataces',
        obraData.id_capataces.map((c) => c.id),
      )
      setSelectedCapataces(obraData.id_capataces.map((c) => c.id))
      setValue(
        'personal',
        obraData.personal_info.personal.map((p) => p.id),
      )
      setSelectedTrabajadores(obraData.personal_info.personal.map((p) => p.id))
    }
  ...
  return (
    <section className="bg-white">
      <div className="container flex-grow justify-center min-h-screen px-6 mx-auto">
        ...
        >
          <section className="w-full grid grid-cols-2 gap-6 mb-4">
          ...
          <div className="mt-4">
            <div className="relative flex items-center">
              ...
              >
                ...
                <h2 className="mx-3 text-gray-400">Foto obra</h2>
                <input
                  id="dropzone-file"
                  type="file"
                  className=""
